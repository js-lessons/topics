...
[Jest](https://facebook.github.io/jest/) &mdash; библиотека для тестирования, разработанная в facebook. В ней есть стандартный набор фич, который есть практически в каждой подобной библиотеке &mdash; возможность создавать моки функций и объектов, набор матчеров, возможность тестировать асинхронный код. Но также Jest пропогандирует интересный подход к тестированию React компонент &mdash; снепшот тестирование. Это подход при котором мы приводим компоненту к некоторому состоянию (например при помощи эмуляции действий пользователя), а потом ее сериализуем и сохраняем в файл (создаем снепшот). После этого этот снепшот становится эталонным и если при следующем запуске теста компонента сериализуется в какое-то другое состояние, то тест не проходит. Если же падение теста произошло из-за преднамеренных действий разработчика, например из-за того, что поменялся тест, то Jest дает возможность перегенерировать снепшот. На самом деле таким образом можно тестировать не только React компоненты, но и любые сериализуемые данные.
