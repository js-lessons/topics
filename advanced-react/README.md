...
Контекст это способ передать данные из компонента родителя во все вложенные компоненты, независимо от уровня вложенности.

Проблема которую контекст решает &mdash; доступ к глобальным данным приложения (локализация, стилизация, глобальный state и т.д.)

Проблемы которые контекст создаёт: 
- Так или иначе контекст является глобальным, а всё что глобальное &mdash; плохо
- Компонент завязанный на котекст становится менее переиспользуемыемым, потому что теперь мы его можем использовтаь только в определённом контексте.
...
Поэтому не рекомендуется использвать контекст, если на это нет действительно весомой причины (например свой фреймворк).
...
[Пример](https://jsfiddle.net/2f0wgku5/6/) HoC, который слушает событие изменения размера окна и передает актуальную высоту окна в обёрнутый компонент.
```jsx harmony
